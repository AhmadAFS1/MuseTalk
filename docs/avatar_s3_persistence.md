...
- If the avatar was prepared with a separate `idle_video_file`, that
  `idle_video.mp4` is included in the same avatar tarball. Single-video avatars
  use `input_video.mp4` for idle playback and do not need a duplicate
  `idle_video.mp4`; S3 upload skips a redundant legacy idle copy when it is byte
  identical to `input_video.mp4`.
- Older single-video avatars still restore with `input_video.mp4` as the idle
  fallback.
