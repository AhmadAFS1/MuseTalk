#!/usr/bin/env python3
"""Measure facial and body consistency of the current Indian-tutor pose set."""
...
ALL = OrderedDict(
    (
        *CURRENT.items(),
        ("speaking_previous", OUTPUT_DIR / "speaking_direct_previous.mp4"),
    )
)
...
def main() -> int:
    ...
    for name in CURRENT:
        ...
        boundary_indexes[name] = [*range(15), *range(count - 15, count)]
        for index in boundary_indexes[name]:
            detect(name, index)
    for index in (0, len(videos["speaking_previous"]) - 1):
        detect("speaking_previous", index)
    ...
    for name in CURRENT:
        ...
        closing = [
            ...
            for index in range(len(videos[name]) - 15, len(videos[name]))
        ]
    ...
    result = {
        ...
        "first_frame_vs_neutral_face_rmse_px": first_vs_neutral,
        "first_frame_vs_neutral_pixels": first_vs_neutral_pixel,
        "speaking_new_vs_previous_face_rmse_px": regional_rmse(
            first_faces["speaking_new"], first_faces["speaking_previous"]
        ),
        "speaking_new_endpoint_face_rmse_px": regional_rmse(
            last_faces["speaking_new"], first_faces["speaking_new"]
        ),
        "speaking_previous_endpoint_face_rmse_px": regional_rmse(
            last_faces["speaking_previous"], first_faces["speaking_previous"]
        ),
    ...
    output_json = OUTPUT_DIR / "geometry_validation.json"
    ...
    cv2.imwrite(str(OUTPUT_DIR / "first_frames_current_and_previous.jpg"), np.hstack(resized))
